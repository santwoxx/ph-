...
export function FluidCursor() {
  ...
  useEffect(() => {
    ...
    function createFBO(w: number, h: number, intFormat: number, format: number, type: number, param: number) {
    ...
    function resizeFBO(target: any, w: number, h: number, intFmt: number, fmt: number, type: number, param: number) {
      const newFBO = createFBO(w, h, intFmt, fmt, type, param);
      copyProgram.bind();
    ...
    // Use only Purple and Yellow
    let colorToggle = false;
    function generateColor(): ColorRGB {
      colorToggle = !colorToggle;
      if (colorToggle) {
        // Deep purple shade
        return { r: 0.15 * 0.8, g: 0.15 * 0.2, b: 0.15 * 1.0 };
      } else {
        // Gold/Yellow shade
        return { r: 0.15 * 1.0, g: 0.15 * 0.9, b: 0.15 * 0.1 };
      }
    }
