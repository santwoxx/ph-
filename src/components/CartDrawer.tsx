...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-acai-100 p-5">
          <h2 className="font-display text-lg font-bold text-acai-950">Seu carrinho</h2>
          <button
            onClick={close}
        ...
        {items.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
            <ShoppingBag className="h-14 w-14 text-acai-200" />
            <p className="font-semibold text-acai-800">Seu carrinho está vazio</p>
            <p className="text-sm text-acai-400">Adicione um açaí delicioso para começar!</p>
          ...
          <>
            <div className="flex-1 space-y-4 overflow-y-auto p-5">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.key}
                    className="flex gap-3 rounded-2xl border border-acai-100 p-3"
                  >
                    <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-acai-50">
                      {item.imageUrl ? (
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-sm font-bold text-acai-950">{item.name}</p>
                        <button
                      ...
                      <p className="text-xs text-acai-400">{item.size}</p>
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <div className="flex items-center gap-2 rounded-full border border-acai-100 px-1.5 py-1">
                          <button
                            onClick={() => decrementItem(item.key)}
                            className="flex h-6 w-6 items-center justify-center rounded-full text-acai-700 hover:bg-acai-50"
                          >
                        ...
                        <span className="text-sm font-bold text-acai-700">
            ...
            <div className="border-t border-acai-100 p-5">
              ...
              <div className="mb-4 flex items-center justify-between">
                <span className="text-sm font-medium text-acai-500">Subtotal</span>
                <span className="font-display text-xl font-extrabold text-acai-950">
                  {formatCurrency(sub)}
                </span>
              </div>
